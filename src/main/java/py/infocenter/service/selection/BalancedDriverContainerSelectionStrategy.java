...
/**
 * select driver container to launcher a driver * selection strategy based on driver container
 * balance * in case the selection driver container died, every time return three candidate for
 * controlcenter if there are enough instance.
 *
 */
public class BalancedDriverContainerSelectionStrategy implements DriverContainerSelectionStrategy {

  private static final Logger logger = LoggerFactory
      .getLogger(BalancedDriverContainerSelectionStrategy.class);

  @Override
  public List<DriverContainerCandidate> select(Collection<Instance> instances,
      List<DriverMetadata> drivers) {
    logger.debug("Going to balance sort instances {} base on existing drivers {}", instances,
        drivers);

    List<DriverContainerCandidate> driverContainerCandidates =
        new ArrayList<DriverContainerCandidate>();

    if (instances == null || instances.size() == 0) {
      return driverContainerCandidates;
    }

    TreeMap<String, DriverContainerBalance> driverBalanceMaps = new TreeMap<String,
        DriverContainerBalance>();
    if (drivers == null) {
    ...
    for (DriverMetadata driverMetadata : drivers) {
      DriverContainerBalance driverContainerBalance = driverBalanceMaps
          .get(driverMetadata.getHostName());
    ...
    for (DriverContainerBalance driverContainerBalance : driverBalanceList) {
      ...
      if (instance != null) {
        ...
        driverContainerCandidate
            .setHostName(instance.getEndPointByServiceName(PortType.CONTROL).getHostName());
        driverContainerCandidate
            .setPort(instance.getEndPointByServiceName(PortType.CONTROL).getPort());
