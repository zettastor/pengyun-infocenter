...
public class InstanceInfoImpl extends InstanceInfo {
  ...
  public InstanceInfoImpl(InstanceId instanceId, Collection<Long> disks, int groupId,
      long freeSpace,
  ...
  private ComparableRebalanceTask selectAnInsideRebalanceTask(List<Long> sourceList,
      List<Long> destList,
  ...
  public ComparableRebalanceTask selectAnInsideRebalanceTask() throws NoNeedToRebalance {
    ...
    if (minAndMax.getFirst() != null) {
      ...
      try {
        return selectAnInsideRebalanceTask(diskList,
            Collections.singletonList(diskWithLeastSegUnits),
            min.getSecond(), true);
    ...
    if (minAndMax.getSecond() != null) {
      ...
      try {
        return selectAnInsideRebalanceTask(Collections.singletonList(diskWithMostSegUnits),
            diskList,
            max.getSecond(), true);
      } catch (NoSuitableTask e) {
        try {
  ...
  private boolean canMoveTo(SimpleSegUnitInfo segUnitInfo, Long destDisk,
      boolean notInTheSameSegmentWrap) {
  ...
  public RebalanceTask selectAprimaryRebalanceTask(Collection<InstanceInfo> destinations)
    ...
    diskListInOrderOfPrimaryCount
        .sort(Comparator.comparingInt(o -> mapDiskToPrimaryUnits.get(o).size()));
    for (InstanceInfo destination : destinations) {
      if (conflictWith((InstanceInfoImpl) destination,
      ...
      while (it.hasNext()) {
        ...
        for (SimpleSegUnitInfo segUnitInfo : mapDiskToPrimaryUnits.get(disk)) {
          if (segUnitInfo.getSegment().getHighestMembership()
              .isSecondary(destination.getInstanceId())) {
  ...
  public String toString() {
    return "InstanceInfoImpl(" + instanceId + " g" + groupId + ")" + ", pressure="
        + calculatePressure()
        + ", primaryPressure=" + primaryPressure() + ", diskCount=" + diskCount + ", freeSpace="
        + freeSpace
        + ", primaryCount=" + primarySegmentUnitCount + ", mapDiskToSegmentUnits="
        + mapDiskToSegmentUnits;
  }
