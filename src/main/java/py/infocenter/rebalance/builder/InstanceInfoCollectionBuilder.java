...
import py.infocenter.rebalance.struct.SimpleSegmentInfo;
import py.infocenter.rebalance.struct.SimpleVolumeInfo;
...
public class InstanceInfoCollectionBuilder {
  ...
  public InstanceInfoCollectionBuilder setMovingOutTasks(
      Multimap<InstanceId, SimpleRebalanceTask> movingOutTasks) {
    this.movingOutTasks = movingOutTasks;
    return this;
  }

  public InstanceInfoCollectionBuilder setMovingInTasks(
      Multimap<InstanceId, SimpleRebalanceTask> movingInTasks) {
    this.movingInTasks = movingInTasks;
    return this;
  }

  public InstanceInfoCollectionBuilder setInsideTasks(
      Multimap<InstanceId, SimpleRebalanceTask> insideTasks) {
    this.insideTasks = insideTasks;
    return this;
  }

  private void buildInstanceSet(Map<InstanceId, InstanceMetadata> instances)
  ...
  private void checkProcessingTasks(Map<InstanceId, InstanceMetadata> instances) {
    ...
    for (SimpleRebalanceTask task : movingOutTasks.values()) {
      if (task.expired()) {
        ...
        SimpleSegmentInfo segment = volumeMap.get(segId.getVolumeId().getId())
            .getSegment(segId.getIndex());
        if (segment.getHighestMembership().isSecondaryCandidate(task.getDestInstanceId())) {
          logger.debug(
              "though the task has been expired, but the target is already secondary candidate in"
                  + " membership {}",
      ...
      if (task.getTaskType() == RebalanceTask.RebalanceTaskType.NormalRebalance
        ...
        if (task.getTaskType() == RebalanceTask.RebalanceTaskType.InsideRebalance) {
          ...
          for (SimpleSegUnitInfo segUnit : mapInstanceToSegmentUnits
              .get(mySegUnit.getInstanceId())) {
        ...
        if (task.getTaskType() == RebalanceTask.RebalanceTaskType.InsideRebalance) {
          bogusUnit = new SimpleSegUnitInfo(mySegUnit.getSegId(),
              destination.getGroup().getGroupId(),
  ...
  private void buildSegmentUnitsMap(Map<InstanceId, InstanceMetadata> instances)
      throws VolumeCreatingException {
    ...
    for (VolumeMetadata volume : volumes) {
      logger.debug("try this volume {} with storage pool {}", volume.getVolumeId(),
      ...
      if (volume.getVolumeStatus() == VolumeStatus.Deleted
      ...
      if (volume.getVolumeStatus() == VolumeStatus.Creating
          || volume.getVolumeStatus() == VolumeStatus.Recycling
