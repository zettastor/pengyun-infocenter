...
public class ZookeeperEnvironmentTool {
  ...
  public void build() throws Exception {
    ...
    String zooKeeperPath =
        currentWorkPath + "/../resources/packages/" + zookeeperPackage + ".tar.gz";
    ...
    for (int i = 1; i <= 3; i++) {
      ...
      String tmpCommand = cmd + configPath;
      logger.debug("command: {}", tmpCommand);
      process = Runtime.getRuntime().exec(tmpCommand);
  ...
  private void createWorkspaceForZookeeper(int index) throws Exception {
    ...
    String cmd = "echo " + index + " > " + dir + "/myid";
